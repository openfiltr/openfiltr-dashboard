    ...
    <ion-content :fullscreen="true">
      <div class="page-shell">
        <page-header
          eyebrow="Filtering"
          title="Block rules"
          subtitle="Manage block-list rules. Search on this page is client-side because the current backend handler only supports pagination."
        >
          <template #actions>
...
const store = useBlockRulesStore()
const ui = useUiStore()
...
async function submit() {
  ...
  if (selectedId.value) {
    ...
    ui.showToast('Block rule updated', 'success')
  } else {
...
async function removeItem(id: string) {
  if (!window.confirm('Delete this block rule?')) {
    return
  }
